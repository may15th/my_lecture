...
# 제가 로그인 view 함수 이름을 login으로 만들어 둬서... 이러면 중복되는데요...
# dust = 60 인데요...
...
def signup(request):
    ...
    if request.method == 'POST':    # 데이터 CREATE 해달라는 뜻
        ...
        # form 인스턴스 만들떄 request.POST에 있는 데이터를 포함해서 만들면?
        form = CustomUserCreationForm(request.POST)
        if form.is_valid(): # 유휴성 검사
...
def login(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            # 요청보낸 유저가 누군지 알아야지 auth_login함수도 실행이 되겠죠?
